/*
 * demo_crc.c — CRC32 integrity check standalone verification demo
 *
 * Compile: aarch64-linux-gnu-gcc -static -O2 demo_crc.c -o demo_crc
 * Run: ./demo_crc
 *
 * CRC-32/ISO polynomial 0xEDB88320 (consistent with Go crc32.ChecksumIEEE)
 * Pure C implementation, no external dependencies.
 */
#include <stdio.h>
#include <string.h>

/* ---- CRC32 table lookup implementation (256 entries) ---- */
...
/* ---- CRC entry structure (consistent with packer side) ---- */

typedef struct {
  unsigned int offset; /* Bytecode segment starting offset */
  unsigned int size;   /* Segment size */
  unsigned int hash;   /* CRC32 checksum value */
} crc_entry_t;
...
static int test_normal(void) {
  /* Calculate CRC for the entire bytecode segment */
  unsigned int expected = crc32_calc(fake_bytecode, sizeof(fake_bytecode));

  /* Simulated CRC table generated by packer */
  crc_entry_t entry;
  entry.offset = 0;
  entry.size = sizeof(fake_bytecode);
  entry.hash = expected;

  /* Simulated stub side verification */
...
static int test_multi_segment(void) {
  /*
   * Simulated multi-segment CRC check (similar to VMProtect segmented CRC)
   * Bytecode is divided into [0..9] and [10..19] segments
   */
...
static int test_trailer_format(void) {
  /*
   * Simulated packing/unpacking of the CRC table at the end of bytecode
   * Format: [crc_magic:u32][crc_count:u32][entries...][original trailer data]
   */
  ...
  /* 3. Parse CRC table (simulated stub side) */
