/*
 * demo_crc_reject.c — CRC verification rejection demo
 *
 * Simulate full stub process:
 *   1. XOR decrypt bytecode
 *   2. Parse BR mapping table (tail)
 *   3. Parse CRC section (before BR table)
 *   4. Verify bytecode CRC + stub memory CRC
 *   5. Verification failed → return 0, do not execute any bytecode
 *
 * Compare with VMProtect leaked source (loader.cc:1910-1935) CRC logic:
 *   VMProtect:  CRCValueCryptor encrypted entry → CalcCRC → is_valid_crc
 *   Ours:       CRC_MAGIC marked section        → crc32_calc → crc_verify
 *
 * Compilation: aarch64-linux-gnu-gcc -static -O2 demo/demo_crc_reject.c -o
 * build/demo_crc_reject
 */
#include <stdio.h>
#include <string.h>

/* =========== Reuse stub CRC implementation (consistent with vm_crc.h) =========== */
...
/* =========== Construct bytecode buffer with CRC section =========== */

/*
 * Simulate bytecode layout generated by packer:
...
static int build_buffer(u8 *buf, int *total_len, u64 stub_va, u32 stub_size) {
  ...
  *total_len = pos;
  return bc_len; /* Return actual bytecode length */
}

/* =========== Simulate stub crc_verify logic =========== */

/*
 * Compare with VMProtect loader.cc:1910-1935:
 *
 * VMProtect:
 *   1. CalcCRC(loader code) == pre-stored hash?     → is_valid_crc
 *   2. Traverse CRC_INFO[] (Decrypt each field),
 *      CalcCRC(region) == crc_info.Hash?            → is_valid_crc
 *   3. !is_valid_crc && CHECK_PATCH →
 *      LoaderMessage(mtFileCorrupted); return LOADER_ERROR;
 *
 * Ours:
 *   1. crc32_calc(bytecode) == bc_crc?              → fail return 0
 *   2. crc32_calc(stub memory) == stub_crc?         → fail return 0
 *   3. Verification failed → return 0 (not running)
 *
 * Differences:
 *   - VMProtect CRC entries themselves are encrypted (CRCValueCryptor), we use magic markers
 *   - VMProtect has a "flag mode" (set_is_patch_detected), we reject directly
 *   - VMProtect checks file CRC (disk file), we only check memory
 */
...
static int test_stub_patch(void) {
  ...
  /* Simulate IDA patching BRK instruction at stub entry */
  u8 saved[4];
  ...
  memcpy(fake_stub, saved, 4); /* Restore */
...
static int test_dual_tamper(void) {
  ...
  buf[0] ^= 0x01;        /* Modify bytecode */
  fake_stub[10] ^= 0xFF; /* Modify stub */
...
int main(void) {
  ...
  printf("VMProtect loader.cc:1929-1932 comparison:\n");
  printf("  VMP:  !is_valid_crc && CHECK_PATCH → return LOADER_ERROR\n");
  printf("  Ours: !crc_verify()               → return 0 (not running)\n\n");
