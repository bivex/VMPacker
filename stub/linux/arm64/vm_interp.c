/*
 * vm_interp.c — Modular VM Interpreter (ARM64 PIC blob)
 */
...
static inline void *sys_mmap(unsigned long size) {
  ...
  register long x2 __asm__("x2") = 7; // PROT_READ | PROT_WRITE | PROT_EXEC
...
static inline void sys_munmap(void *addr, unsigned long size) {
  register long x8 __asm__("x8") = 215; // sys_munmap
...
__attribute__((section(".data.entry"), used)) volatile u64 _token_table_va[3] = {0, 0, 0};
