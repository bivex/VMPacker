...
static inline u32 h_fcmp(vm_ctx_t *vm) {
  ...
  if (type == 0) {
    ...
    else __builtin_memcpy(&fb, &vm->V[m & 31][0], 4);
    ...
    __builtin_memcpy(&da, &vm->V[n & 31][0], 8);
    if (m == 16) db = 0.0;
    else __builtin_memcpy(&db, &vm->V[m & 31][0], 8);
...
static inline u32 h_fcvt_if(vm_ctx_t *vm) {
  ...
  if (fp_type == 0) { /* To float */
    ...
    __builtin_memcpy(&vm->V[d & 31][0], &f, 4);
  } else { /* To double */
    double db;
    if (sf) db = (double)(i64)val;
    else db = (double)(i32)val;
    __builtin_memcpy(&vm->V[d & 31][0], &db, 8);
  }
  return 4;
}
...
static inline u32 h_fcvt_fi(vm_ctx_t *vm) {
  ...
  if (fp_type == 0) { /* From float */
    float f; __builtin_memcpy(&f, &vm->V[n & 31][0], 4);
    if (sf) vm->R[d & 31] = (u64)(i64)f;
    else vm->R[d & 31] = (u64)(u32)(i32)f;
  } else { /* From double */
    double db; __builtin_memcpy(&db, &vm->V[n & 31][0], 8);
    if (sf) vm->R[d & 31] = (u64)(i64)db;
    else vm->R[d & 31] = (u64)(u32)(i32)db;
  }
  return 4;
}
...
static inline u32 h_fneg(vm_ctx_t *vm) {
  u8 d = vm->bc[vm->pc + 1], n = vm->bc[vm->pc + 2], type = vm->bc[vm->pc + 3];
  if (type == 0) {
    float v; __builtin_memcpy(&v, &vm->V[n & 31][0], 4);
    v = -v; __builtin_memcpy(&vm->V[d & 31][0], &v, 4);
  } else {
    double v; __builtin_memcpy(&v, &vm->V[n & 31][0], 8);
    v = -v; __builtin_memcpy(&vm->V[d & 31][0], &v, 8);
  }
  return 4;
}
...
static inline u32 h_fabs(vm_ctx_t *vm) {
  ...
  if (type == 0) {
    float v; __builtin_memcpy(&v, &vm->V[n & 31][0], 4);
    if (v < 0) v = -v;
    __builtin_memcpy(&vm->V[d & 31][0], &v, 4);
  } else {
    double v; __builtin_memcpy(&v, &vm->V[n & 31][0], 8);
    if (v < 0) v = -v;
    __builtin_memcpy(&vm->V[d & 31][0], &v, 8);
  }
  return 4;
}
...
static inline u32 h_fmov_rv(vm_ctx_t *vm) {
  ...
  if (type == 0) { /* 32-bit */
    u32 v32 = (u32)val;
    __builtin_memcpy(&vm->V[d & 31][0], &v32, 4);
  } else { /* 64-bit */
    __builtin_memcpy(&vm->V[d & 31][0], &val, 8);
  }
  return 4;
}
...
static inline u32 h_fmov_vr(vm_ctx_t *vm) {
  ...
  u64 val = 0;
  if (type == 0) { /* 32-bit */
    u32 v32; __builtin_memcpy(&v32, &vm->V[n & 31][0], 4);
    val = (u64)v32;
  } else { /* 64-bit */
    __builtin_memcpy(&val, &vm->V[n & 31][0], 8);
  }
  vm->R[d & 31] = val;
  return 4;
}
